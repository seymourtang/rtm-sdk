...
func Context(signal <-chan struct{}) context.Context {
	ret, _ := ContextWithCancel(signal)
	return ret
}

// ContextWithCancel returns a Context and CancelFunc, the Context is cancelled upon the given signal
func ContextWithCancel(signal <-chan struct{}) (context.Context, context.CancelFunc) {
