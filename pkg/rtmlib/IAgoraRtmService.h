...
namespace agora {
  namespace rtm {
    ...
    class IWrappedIChannelEventHandler : public IChannelEventHandler {
    public:
            virtual ~IWrappedIChannelEventHandler() {

            }

            virtual void onWrappedGetMembers(IChannelMember *member, int count, GET_MEMBERS_ERR errorCode) = 0;

            virtual void onGetMembers(IChannelMember **members, int userCount, GET_MEMBERS_ERR errorCode) override {
                    if (!members || userCount <= 0) onWrappedGetMembers(nullptr, userCount, errorCode);

                    for (int i = 0; i < userCount; i++) {
                            if (members[i]) onWrappedGetMembers(members[i], userCount, errorCode);
                            else onWrappedGetMembers(nullptr, userCount, errorCode);
                    }
            }
    };
    /**
